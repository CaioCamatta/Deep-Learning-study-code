...
# Backup session. Prevents from losting performance
session = None
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    for epoch in range(n_epochs):
        ...
        # If performance goes up, save batch, otherwise run again
        if np.amax(rewards_from_batches) > max_reward_recorded:
            session = sess
        else:
            sess = session
